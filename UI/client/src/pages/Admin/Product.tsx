        ...
        try {
            ...
            setAttributes(
                (detail.attributes ?? []).map((a: any) => ({
                    name: a.name ?? "", value: a.value ?? "", unit: a.unit ?? "", description: a.description ?? ""
                }))
            );
        } catch {
            setFormData({ ...initialFormData, name: product.name, img: product.img, price: product.price, unit: product.unit ?? "", inStock: product.inStock, supplier: { name: product.supplier.name, address: product.supplier.address } });
            setAttributes([]);
        } finally {
            setIsLoading(false);
            setIsOpen(true);
