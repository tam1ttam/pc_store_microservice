...
raw = p.read_bytes()
print("Raw hex head:", raw[:60].hex())
print("Raw decode:", raw[:120].decode('utf-8', errors='replace')[:120])

# Write the fixed file
content = '''"""LangChain Tools for the AI agent.
...
def get_product_info(product_id: str) -> str:
    ...
    try:
        ...
        return (
            f"{p.get('name')}\n"
            f"Gia: {p.get('price', 0):,.0f}d | Don vi: {p.get('unit') or '--'}\n"
            f"Ton kho: {stock if stock >= 0 else 'khong ro'}\n"
            f"{'Thong so: ' + attrs if attrs else ''}"
        )
    except Exception as exc:
...
print(f"Written {len(written)} bytes")
print("First 200 chars:", written[:200])
