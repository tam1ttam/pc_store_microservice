...
export default function CartSummary({
  ...
  return (
    <div className="bg-white rounded-xl p-6 border border-gray-200 sticky top-4">
      <h3 className="text-lg font-bold text-gray-800 mb-4">{t('cartSummary.title')}</h3>

      <div className="space-y-3 mb-6">
        <div className="flex justify-between text-gray-600">
          <span>{t('cartSummary.subtotal', { count: totalItems })}</span>
          <span className="font-medium">{formatPrice(subtotal)}</span>
        </div>
        {savings > 0 && (
          <div className="flex justify-between text-gray-600">
            <span>{t('cartSummary.savings')}</span>
            <span className="text-green-600">-{formatPrice(savings)}</span>
          </div>
        )}
        <div className="flex justify-between text-gray-600">
          <span>{t('cartSummary.shipping')}</span>
          <span className="text-green-600">{shipping === 0 ? t('cartSummary.free') : formatPrice(shipping)}</span>
        </div>
      ...
      >
        <ShoppingCart className="w-5 h-5" /> {t('cartSummary.checkout')}
      </button>

      <p className="text-xs text-gray-500 text-center mt-4">
        {t('cartSummary.freeShippingNote')}
      </p>
    </div>
  );
}
