...
import { fetchPendingOrders } from "@/redux/thunks/orderManage";
...
export default function Header() {
	...
	const { pendingOrders } = useSelector((state: RootState) => state.orderManage);
	...
	const handleLogout = async () => {
		try {
			...
			if (result.payload.code === 1000) {
				...
				toast({ title: t('auth.logoutFailed') });
			}
		} catch {
			toast({ title: t('auth.logoutFailed') });
		}
			...
			<header
				...
				style={{
					boxShadow: isScrolled
						? ` 0 0 10px rgba(251,146,60,0.6), inset 0 0 40px rgba(251,146,60,0.3), 0 0 20px rgba(251,146,60,0.2), 0 0 30px rgba(251,146,60,0.15), 0 0 40px rgba(251,146,60,0.1) `
						: "none",
					border: isScrolled ? `1px solid rgba(251,146,60,0.4)` : "none",
					backgroundImage: isScrolled
						? `linear-gradient(135deg, rgba(251,146,60,0.18) 0%, rgba(225,29,72,0.18) 25%, rgba(147,51,234,0.18) 50%, rgba(225,29,72,0.18) 75%, rgba(251,146,60,0.18) 100% )`
						: "none",
						...
						{isLogin ? (
							<div className="flex items-center gap-2 sm:gap-4">
								...
								<div className="relative">
									...
									{pendingCount > 0 && (
										<div
											className="absolute -bottom-1 -right-1 bg-yellow-500 text-white text-[8px] sm:text-[10px] font-bold rounded-full min-w-[16px] sm:min-w-[18px] h-[16px] sm:h-[18px] flex items-center justify-center ring-2 ring-black/30"
											title={`${pendingCount} đơn chờ xác nhận`}
										>
